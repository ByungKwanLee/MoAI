...
_PREDEFINED_SPLITS_PRETRAIN = {
    "textvqa_val": ["llava_textvqa_val_v051_ocr.json"],
}

def get_metadata(name):
    if name in ['textvqa_val']:
        return {'gt_json': os.path.join(_coco_root, 'TextVQA/TextVQA_0.5.1_val.json')}

evaluator_mapper = {'textvqa_val': 'textvqa'}
...
def register_pretrain(
    ...
    semantic_name = name
    arrow_root = os.path.join(arrow_root, 'TextVQA')
    if os.path.exists(arrow_root):
        ...
        # logger.warning("WARNING: Cannot find TextVQA Dataset. Make sure datasets are accessible if you want to use them for training or evaluation.")        
