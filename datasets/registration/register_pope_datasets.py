...
def get_metadata(name):
    if name in ['pope_test']:
        return {'gt_json': os.path.join(_coco_root, 'POPE/new_pope_test.json')}

evaluator_mapper = {'pope_test': 'pope'}
...
def register_pretrain(
    ...
    semantic_name = name
    arrow_root = os.path.join(arrow_root, 'POPE')
    if os.path.exists(arrow_root):
        ...
        # logger.warning("WARNING: Cannot find POPE Dataset. Make sure datasets are accessible if you want to use them for training or evaluation.")        
