...
def get_metadata(name):
    if name in ['hallusionbench']:
        return {'gt_json': os.path.join(_coco_root, 'HallusionBench.json')}

evaluator_mapper = {'hallusionbench': 'hallusionbench'}
...
def register_pretrain(
    ...
    semantic_name = name
    arrow_root = os.path.join(arrow_root, 'HallusionBench')
    if os.path.exists(arrow_root):
        ...
        logger.warning("WARNING: Cannot find HallusionBench Dataset. Make sure datasets are accessible if you want to use them for training or evaluation.")        
