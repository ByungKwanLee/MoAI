...
_PREDEFINED_SPLITS_PRETRAIN = {
    "qbench_test": ["llvisionqa_test.json"],
    "qbench_dev": ["llvisionqa_dev.json"],
    "qbench_cn_test": ["质衡-问答-测试集.json"], 
    "qbench_cn_dev": ['质衡-问答-验证集.json']
}

def get_metadata(name):
    if name in ['qbench_test']:
        return {'gt_json': os.path.join(_coco_root, 'LLVisionQA-QBench/llvisionqa_test.json')}
    elif name in ['qbench_dev']:
        return {'gt_json': os.path.join(_coco_root, 'LLVisionQA-QBench/llvisionqa_dev.json')}
    elif name in ['qbench_cn_test']:
        return {'gt_json': os.path.join(_coco_root, 'LLVisionQA-QBench/质衡-问答-测试集.json')}
    elif name in ['qbench_cn_dev']:
        return {'gt_json': os.path.join(_coco_root, 'LLVisionQA-QBench/质衡-问答-验证集.json')}
    else:
        return {}

evaluator_mapper = {'qbench_test': 'qbench', 'qbench_dev': 'qbench', 'qbench_cn_test': 'qbench', 'qbench_cn_dev': 'qbench'}
...
def register_pretrain(
    ...
    semantic_name = name
    arrow_root = os.path.join(arrow_root, 'LLVisionQA-QBench')
    if os.path.exists(arrow_root):
        ...
        logger.warning("WARNING: Cannot find QBench Dataset. Make sure datasets are accessible if you want to use them for training or evaluation.")        
