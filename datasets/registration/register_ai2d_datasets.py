...
def get_metadata(name):
    if name in ['ai2d']:
        return {'gt_json': os.path.join(_coco_root, 'ai2d_test.json')}

evaluator_mapper = {'ai2d': 'ai2d'}
...
def load_pretrain_data(arrow_root, meta, name, pretrain_arrows):
    ...
    for question in questions:
        ...
        image_id = question['imageName']
        text = question['question']
        question_id = question['metadata']['questionId']
...
def register_pretrain(
    ...
    semantic_name = name
    arrow_root = os.path.join(arrow_root, 'ai2d')
    if os.path.exists(arrow_root):
        ...
        logger.warning("WARNING: Cannot find AI2D Dataset. Make sure datasets are accessible if you want to use them for training or evaluation.")        
