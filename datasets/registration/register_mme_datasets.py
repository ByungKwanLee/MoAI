...
def get_metadata(name):
    if name in ['mme']:
        return {'gt_json': os.path.join(_coco_root, 'MME_Benchmark_release_version/llava_mme.json')}

evaluator_mapper = {'mme': 'mme'}
...
def register_pretrain(
    ...
    semantic_name = name
    arrow_root = os.path.join(arrow_root, 'MME_Benchmark_release_version')
    if os.path.exists(arrow_root):
        ...
        # logger.warning("WARNING: Cannot find MME Dataset. Make sure datasets are accessible if you want to use them for training or evaluation.")        
