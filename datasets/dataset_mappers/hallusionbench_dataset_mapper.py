...
__all__ = ["HallusionBenchDatasetMapper"]
...
class HallusionBenchDatasetMapper:
    """
    ...
    def get_image(self, inp):
        ...
        try: #### correct some file name problem
            image_file = "HallusionBench/hallusion_bench/{}".format(inp[2:])
            image = Image.open(os.path.join(_root, image_file)).convert('RGB')
        except:
            if inp.find('png') != -1:
                image_file = "HallusionBench/hallusion_bench/{}".format(inp[2:]).replace('png', 'PNG')
            else:
                image_file = "HallusionBench/hallusion_bench/{}".format(inp[2:]).replace('PNG', 'png')
            image = Image.open(os.path.join(_root, image_file)).convert('RGB')
    ...
    def __call__(self, dataset_dict):
        ...
        image = self.get_image(dataset_dict['filename'])
