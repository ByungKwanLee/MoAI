...
class Transformer(BaseModule):
    ...
    def forward(self, x, mask, query_embed, pos_embed):
        ...
        # (bs, num_queries, dim)
        x = x.view(bs, c, -1).permute(0, 2, 1)  # [bs, c, h, w] -> [bs, h*w, c]
        pos_embed = pos_embed.view(bs, c, -1).permute(0, 2, 1)
        query_embed = query_embed.unsqueeze(1).repeat(
            1, bs, 1).permute(1, 0, 2)  # [num_query, dim] -> [bs, num_query, dim]
        ...
        out_dec = self.decoder(
            ...
            key_padding_mask=mask)
        memory = memory.permute(0, 2, 1).reshape(bs, c, h, w)
...
class PSGTrHead(AnchorFreeHead):
    ...
    def __init__(
        ...
        self.obj_loss_cls = MODELS.build(obj_loss_cls)
        self.obj_loss_bbox = MODELS.build(obj_loss_bbox)
        self.obj_loss_iou = MODELS.build(obj_loss_iou)

        self.sub_loss_cls = MODELS.build(sub_loss_cls)
        self.sub_loss_bbox = MODELS.build(sub_loss_bbox)
        self.sub_loss_iou = MODELS.build(sub_loss_iou)
        if self.use_mask:
            self.obj_dice_loss = MODELS.build(obj_dice_loss)
            self.sub_dice_loss = MODELS.build(sub_dice_loss)

        self.rel_loss_cls = MODELS.build(rel_loss_cls)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        convert_dict = {
            ...
            # '.decoder.post_norm.': '.decoder.post_norm.',
    ...
    def predict_by_feat(self, *feats, batch_img_metas, rescale):
        ...
        masks = F.interpolate(masks.unsqueeze(1).to(torch.float16),
        ...
        # bbox2_results = [bbox2result(all_cls_scores, all_bbox_preds, self.num_classes)]
    ...
    def _get_bboxes_single(self,
        ...
        if self.use_mask:
            ...
            s_mask_pred = F.interpolate(s_mask_pred.unsqueeze(1).to(torch.float16),
                                        size=mask_size).squeeze(1).to(torch.bfloat16)
            o_mask_pred = F.interpolate(o_mask_pred.unsqueeze(1).to(torch.float16),
                                        size=mask_size).squeeze(1).to(torch.bfloat16)
        ...
        if rescale:
            s_det_bboxes /= s_det_bboxes.new_tensor(scale_factor*2)
        ...
        if rescale:
            o_det_bboxes /= o_det_bboxes.new_tensor(scale_factor*2)
